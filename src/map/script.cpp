	...
	switch(id){
		...
		case 1:	// HP / SP Potions
			...
			if(index == -1) { // Show the list of all potions set
				for (i = 0; i < MAX_INVENTORY; i++) {
					// Find item data
					if ((item_data = item_db.find(sd->inventory.u.items_inventory[i].nameid)) == nullptr)
						continue; // Skip invalid items

					if (item_data->type == IT_HEALING) { // Check if item is usable
						// Check if the item is already active
						bool is_active = std::any_of(
							sd->aa.autopotion.begin(),
							sd->aa.autopotion.end(),
							[item_id = item_data->nameid](const s_autopotion &v) { return v.item_id == item_id; });

						// Add to the menu
						if (is_active) {
							// If the item is active, mark it as [ON]
							buf += "^0000FF[ON]^000000 (" + std::to_string(item_data->nameid) + ") " + item_data->ename + ":";
						} else {
							// If the item is not active, mark it as [OFF]
							buf += "^FF0000[OFF]^000000 (" + std::to_string(item_data->nameid) + ") " + item_data->ename + ":";
						}
					}
				}
				...
				if (!sd->aa.autopotion.empty()) {
					for (auto &itAutopotion : sd->aa.autopotion) {
						// Find item data for the active item
						item_data = item_db.find(itAutopotion.item_id);
						if (item_data) {
							// Add active items to the menu
							buf += "^0000FF[ON]^000000 (" + std::to_string(itAutopotion.item_id) + ")^000000 " + item_data->ename.c_str() + " ";
			...
			switch(index){
				case 0:
					if(sd->aa.autositregen.is_active)
						buf += std::string("^0000FF(Enabled)^000000 \n");
					else
						buf += std::string("^FF0000(Disabled)^000000 \n");
					break;
				case 1:
					if(sd->aa.autositregen.min_hp > 0)
						buf += "Your character will take a ^FF0000Sit^000000 to rest if HP < " + std::to_string(sd->aa.autositregen.min_hp) + 
								"% - ^FF0000Stand^000000 if HP >= " + std::to_string(sd->aa.autositregen.max_hp) + "% \n";
					break;
				case 2:
					if(sd->aa.autositregen.min_sp)
						buf += "Your character will take a ^FF0000Sit^000000 to rest if SP < " + std::to_string(sd->aa.autositregen.min_sp) + 
								"% - ^FF0000Stand^000000 if SP >= " + std::to_string(sd->aa.autositregen.max_sp) + "% \n";
					break;
			}
			...
			if(index == -1){ // Show all buff skills available
				for(i=0;i<MAX_SKILL;i++){
					if(sd->status.skill[i].id > 0 && sd->status.skill[i].lv > 0){
						...
						if(skill 
							...
							//Remove skill from menu if it's already active.
							for (auto &itAutobuffskills : sd->aa.autobuffskills){
								if (itAutobuffskills.skill_id == sd->status.skill[i].id) {
									is_active = true;
									break;
								}
							}								
							// Add menu option with active status
							if (is_active) {
								buf += std::string("^0000FF[ON] ^000000(") + std::to_string(skill->nameid) + ") " + skill->desc + 
									   " - Lv. [" + std::to_string(sd->status.skill[i].lv) + "]:";
							} else {
								buf += std::string("^FF0000[OFF] ^000000(") + std::to_string(skill->nameid) + ") " + skill->desc + 
									   " - Lv. [" + std::to_string(sd->status.skill[i].lv) + "]:";
							}
						}
					}
				...
				if(sd->aa.autobuffskills.size() > 0){
					for(auto &itAutobuffskills : sd->aa.autobuffskills){
						...
						if(skill)
							buf += std::string("^0000FF[ON] (") + std::to_string(itAutobuffskills.skill_id) + ")^000000 " + skill->desc + 
									" - Lv. [" + std::to_string(itAutobuffskills.skill_lv) + "]:";
					}
				}
			}
			...
			if(index == -1){ // Show all attack skills available
					...
					if(sd->status.skill[i].id > 0 && sd->status.skill[i].lv > 0){
						...
						if (skill 
							...
							for (auto &itAutoattackskills : sd->aa.autoattackskills) {
								if (itAutoattackskills.skill_id == sd->status.skill[i].id) {
									is_active = true;
									break;
								}
							}
				...
				if(sd->aa.autoattackskills.size() > 0){
					for(auto &itAutoattackskills : sd->aa.autoattackskills){
						...
						if (skill) {
							buf += std::string("^0000FF[ON] ^000000(") + std::to_string(skill->nameid) + ") " + skill->desc +
								   " - Lv. [" + std::to_string(itAutoattackskills.skill_lv) + "]:";
						} else {
							buf += "No active skills.";
						}
					}
				}
			}
		...
		case 5:	// Buff items
			...
			if (index == -1) { // List all buff items in the inventory
				for (i = 0; i < MAX_INVENTORY; i++) {
					// Find item data
					if ((item_data = item_db.find(sd->inventory.u.items_inventory[i].nameid)) == nullptr)
						continue; // Skip invalid items

					if (item_data->type == IT_USABLE) { // Check if item is usable
						// Check if the item is already active
						bool is_active = std::any_of(
							sd->aa.autobuffitems.begin(),
							sd->aa.autobuffitems.end(),
							[item_id = item_data->nameid](const s_autobuffitems &v) { return v.item_id == item_id; });
			...
			} else if (index == 0) { // Show active buff items
				if (!sd->aa.autobuffitems.empty()) {
					for (auto &itAutobuffitem : sd->aa.autobuffitems) {
						// Find item data for the active item
						item_data = item_db.find(itAutobuffitem.item_id);
						if (item_data) {
							// Add active items to the menu
							buf += "^0000FF[ON]^000000 (" + std::to_string(item_data->nameid) + ") " + item_data->ename + ":";
						}
...
BUILDIN_FUNC(getautoattackint)
{
	...
	switch(id){
		case 0:	// auto heal
			...
			if(index == 0)
				num = static_cast<int>(sd->aa.autoheal.size());
		...
		case 3:	// Active auto buffs skills
			if(index == 0)
				num = static_cast<int>(sd->aa.autobuffskills.size());

			script_pushint(st, num);
			break;
		case 4:	// Active auto attack skills
			if(index == 0)
				num = static_cast<int>(sd->aa.autoattackskills.size());

			script_pushint(st, num);
			break;
		case 5:	// Buff items
			if(index == 0)
				num = static_cast<int>(sd->aa.autobuffitems.size());
		...
		case 8: // Monster list
			switch(index){
				case 0:
					...
					break;
				case 1:
					num = static_cast<int>(sd->aa.mobs.id.size());
					break;
			}
		...
		case 9: // Item list
			switch(index){
				case 1:
					num = static_cast<int>(sd->aa.pickup_item_id.size());
					break;
				case 2:
